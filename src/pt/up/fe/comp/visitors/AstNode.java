package pt.up.fe.comp.visitors;

public final class AstNode {
    public static final String START = "Start";
    public static final String PROGRAM = "Program";
    public static final String IMPORT_DECL = "Import";
    public static final String CLASS_DECL = "ClassDeclaration";
    public static final String METHOD_DECL = "MethodDeclaration";
    public static final String METHOD_HEADER = "MethodHeader";
    public static final String MAIN_DECL = "MainDeclaration";
    public static final String VAR_DECL = "VarDeclaration";
    public static final String PARAM_DECL = "ParameterDeclaration";
    /*public static final String
    public static final String
    public static final String
    public static final String
*/
}
